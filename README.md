...
- **电网数据**：长三角真实 OSM 数据（**22,233 条电力要素**：线路 17,967 / 变电站 3,529 / 电厂 733 /
  **换流站 4**），本地切为 **7.12 MB** PMTiles（`osm_grid.pmtiles`，7,465,373 字节）随安装包分发。
  历次变化：阶段43 含铁路管道 5.56 MB → 阶段56-A1 撤销铁路/油气管道并扩容属性 7.02 MB
  → 阶段56-A2 加入 `frequency`/`is_dc`/换流站 7.12 MB
...
- **GEM 发电设施**（阶段48-A 引入，阶段50 改为 PMTiles 数据包）：Global Energy Monitor
  ...
  **33,790 机组聚合为 14,793 座电站**（煤炭 4,865 / 油气 6,390 / 生物质 3,538），
  ...
  详见下文「数据来源与许可」
- **图层控制**：`电厂 / 变电站 / 换流站 / 输电线路 / 海底电缆` 五个总开关；输电线路再按 `735kV 以上 /
  500-734kV / 220-499kV / 220kV 以下 / 电压未知 / 直流（HVDC）` **六个**原生复选框细分
  （**「电压未知」与「220kV 以下」默认关闭**；直流档默认开启）。
  阶段56-A2 起：直流按 `is_dc` 分档（虚线红），交流五档已排除电缆与直流线；
  海底电缆是 `power=cable` 单独成层（青绿实线）
...
| 长三角 OSM 电网切片 | `maps\osm_grid.pmtiles` | 7.12 MB（7,465,373 B） |
...
- **已内置真实数据**（不再是占位，阶段37–39 完成）：
  ...
  - **全国 7 大区域电网数据包** —— OSM **纯电力要素**（线路 / 变电站 / 电厂 / 换流站）共
    **381,545 个**（合并后；合并前 **431,031** = A1 口径 430,969 + 62 个换流站，逐位对得上）、
    合计 **154.96 MB**（162,486,471 字节），以 PMTiles 存于 `data/packs/osm-<region>.pmtiles`
    （**不进 Git**），前端按需加载。
    ...
    ✅ 阶段56-A2：新增 `frequency` / `is_dc`（直交流分档）+ **换流站**图层 + **海底电缆**图层；
    因为新增了 3 个属性/图层，7 包体积从 151.61 → **154.96 MB**（其中 `length_km`/`osm_ids` 是大头，
    见 `README_OSM.md` 的阶段56-A2 一节）。
    ⚠️ **清单（`packs_manifest.json`）尚未重算** —— 指纹与体积仍是 A1 那一版，
    `verify_packs.mjs --remote` 会报不符，属**预期**（重算与重传在 A3）。
  - **直交流分档的诚实边界**：`is_dc` 按设计 §4.1 三步判定（`frequency=="0"` → 端点接换流站 →
    默认按交流）。OSM 的 `frequency` 覆盖率实测约 22–40%（分区域），
    ⇒ **未识别的直流线路会按交流显示**，前端图例已写明这一点，不暗示"直流已全部识别"。
- 地图支持**按需加载**与**空间查询**：
