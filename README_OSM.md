...
> ✅ **阶段56-A1 复核（2026-09-24）**：上表本身就是**电力要素**口径，重建后
> **要素数、瓦片数、最大单瓦片逐项一致**（430,969 / 167,974 / 403.7 KB）——
> 这是对"纯电力口径"的一次独立复核，而不是新造数字。
> 唯一变化是**体积**：合计 **116.52 → 123.15 MiB**，因为阶段56-A1 在同一次重建里
> **扩容了电力属性**（`ref` / `operator` / `cables` / `wires` / `circuits` / `plant_output`）。
...
> 🔴 **整节已作废（阶段56-A1，2026-09-24）**：下图是**含铁路/油气管道的"全要素"**口径。
> 用户决定**只做电力**后，这两类数据的**显示与数据一并移除** ——
> 抓取类别、prepare 合并表、切片白名单、前端图层/面板/图例全部删除，
> 8 个归档（7 区域包 + 核心区）已按纯电力口径重建。
>
> **当前真值**：核心区 `osm_grid.pmtiles` **25,611 要素 / 5.79 MiB**；
> 7 个区域包合计 **430,969 要素 / 123.15 MiB**（详见上方阶段38 表格的复核说明）。
> 下面保留的是**历史记录**，其中的"全要素"体积与铁路/管道条数**不再成立**。
