...
### 性能：50ms 护栏告警不是本阶段引入的

dev 日志出现 `⚠️ 视野统计超过 50ms 护栏：90.8ms / 58.9ms`。我没有假设是自己引入的，
而是做了对照测量（同样跑 `moveend` 后的统计）：
...
**带 2 个区域包的四川反而比只画核心区的长三角更快** ⇒ 告警来自长三角那份密集数据的既有开销，
与本阶段无关。低缩放（z1.5 启动时）单张瓦片装了 ~2.5 万个要素，是真正的耗时来源。
