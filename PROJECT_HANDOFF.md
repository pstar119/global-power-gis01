...
| 7 | 重打安装包 | ✅ | `Global Power GIS_0.2.0_x64-setup.exe` = **48,128,414 B / 45.90 MiB**（红线 50 MB）；`check_release_redlines.mjs --with-exe` 通过。⚠️ 核心区归档 +0.10 MB 而安装包 −0.22 MiB —— 别用单资源增量推安装包增量 |

> ✅ **两项复测已完成**（2026-09-25，CDP 打通后实测，见设计文档 §9.2/§9.3）：
> · **图层面板 1280×800 默认态（图层控制+图例都折叠）= 416/416 ⇒ 溢出 0，验收口径通过**；
>   全展开态（图层控制+电力设施+电压分级+当前视野+数据看板）仍滚动 249/389px —— 与阶段54 的
>   "65px"不可直接比（那次没把这些一起展开）。
> · **G8「需更新」在真实实例里验证通过**：设置页 5 行显示「需更新」+ 更新按钮 + 本机/清单字节数凭据，
>   GEM 显示「已下载」；`verify_packs.mjs --remote` 三段（data/packs / 用户目录 / 远端）**全绿、退出码 0**。
> · 期间包目录里出现过一次"7 个包在 12 秒内被更新"（华南失败留下 0 字节 `.part`）：用探针排除了
>   "应用启动自动下载"与"CDP 点击触发"，确认是**用户在界面上自己点的更新**；华南已用
>   `install_packs.mjs --user-dir --only huanan` 补齐。
> · 📌 供以后复用：Tauri 一旦设了 `additionalBrowserArgs`，WebView2 就**忽略**
>   `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 环境变量 ⇒ 要开 CDP 只能**临时**改
>   `tauri.conf.json` + **只构建 debug**，量完 `git checkout` 还原（红线自检有 `9222` 禁用串兜底）。
...
| 项 | 值 | 核实 |
|---|---|---|
| 安装包体积 | ⚠️ **易失，不写正文** —— 用 §10「产物（现查）」现查。阶段56-A3 重打包基线：`48,128,414 B = 45.90 MiB`（裸 exe `9.48 MiB`），余量约 4.10 MiB | ✅ 阶段56-A3 实测（阶段55 基线 46.12 MiB → 本次 −0.22 MiB，而核心区归档反而 +0.10 MB） |
