...
| 数字 | 正确口径 | 性质 |
|---|---|---|
| `430,969` | **纯电力要素**（线路 / 变电站 / 电厂） | ✅ 稳定，长期有效（阶段56-A1 重建后逐区复核仍成立） |
| ~~`815,728`~~ | ~~清单全要素（含铁路 / 油气管道）~~ | 🔴 **已作废（阶段56-A1）** —— 铁路/管道整体撤销，该口径不再存在 |
| `34,936` | WRI 电厂行数（seed 库 `power_plants`） | ✅ 稳定 |
| ~~`4.68 MB`~~ | 长三角核心区切片体积 | 🔴 **已作废** —— 阶段56-A1 重建实测 `osm_grid.pmtiles` = **5.79 MiB**（撤销铁路/管道后从 7.49 回落；又因属性扩容略高于更早的 4.68） |
| 安装包体积 / SHA256 | — | ⚠️ **易失**：每次重建都变。**不要写进正文**，用 §10「产物（现查）」的命令现查 |

> 🔴 **口径铁律（阶段56-A1 更新）**：现在只有**一个**口径 —— 电力要素 **`430,969`**。
...
| 全国 7 区域 OSM 电网 | **电力要素 430,969 个 / 123.15 MiB**<br>（华东 93,559 · 华中 57,576 · 华南 45,295 · 华北 62,820 · 东北 29,948 · 西南 73,055 · 西北 68,716） | ✅ **阶段56-A1 重建实测**：7 个包逐个重建、逐区比对，要素数与括号内数字**逐一吻合**。<br>体积略大于撤销前同口径的 116.52 MB —— 因为本次**扩容了属性**（ref / operator / cables / wires / circuits / plant_output） |
...
| 核心区电网 | `resources/maps/osm_grid.pmtiles`，**5.79 MiB**（阶段56-A1 重建实测，25,611 要素），bbox `[118,27,123,33]`（长三角+浙江） | ✅ 实测：撤销铁路/管道后从 7.49 MiB 回落 |

> 📌 **口径已归一**：阶段56-A1 起只有「电力要素」一个口径。
> 旧文档并列的「全要素 / 纯电力」两套数字**已成历史**，不要再并列引用。
...
1. **上传 `gem-plants.pmtiles`**（🔴 **最高优先级 —— 当前唯一的功能性断点**）。
