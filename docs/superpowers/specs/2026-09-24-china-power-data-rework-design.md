...
### 9.2 两项复测（2026-09-25，CDP 打通后**实测**，不再是预期）

**方法（供以后复用）**：Tauri 的 WindowConfig **一旦设置了 `additionalBrowserArgs`**，
WebView2 就会**忽略** `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 环境变量（本机实测：设了环境变量
9222 也不监听）。所以只能在 `tauri.conf.json` 的窗口里**临时**加
`"additionalBrowserArgs": "--remote-debugging-port=9222 …"`，并**只构建 debug**
（`tauri build --debug --no-bundle`，release 产物一个字节都不动），量完立刻 `git checkout` 还原。
红线自检里有 `9222` / `remote-debugging` 两条禁用串，漏进发布产物会被当场抓到。

**(1) 图层面板溢出（1280×800，视口实测就是 1280×800）**

| 状态 | 面板 clientHeight / scrollHeight | 溢出 |
|---|---|---|
| **① 默认态（图层控制 + 图例都折叠）** | **416 / 416** | **0 px ⇒ ✅ 验收口径通过** |
| ② 展开「图例」 | 530 / 530 | 0 |
| ③ 收起图例 + 展开「图层控制」（电力设施默认展开） | 553 / 802 | 249 px |
| ④ 再展开「电压分级」（第 3 层） | 553 / 942 | 389 px |

⚠️ 与阶段54 记的"展开该层仍滚动 65px"**不可直接比**：那次面板里只有图层控制相关项，
本次 ③④ 时「当前视野」（多行）与「数据看板」（15 类燃料）也同时展开着（整份内容本来就有
~1700px，README 阶段47 已记）。**验收口径是默认态，已通过。**

**(2) G8 在真实运行实例里的呈现**（设置 → 数据包管理）

- 5 行显示 **「需更新」徽标 + 「更新」按钮 + 「本机 14.0 MB ≠ 清单 13.1 MB」凭据**（华南/华北/东北/西南/西北）；
  GEM 显示「已下载」+ 删除 ⇒ **与 9.1 的数值预期完全一致**，UI 路径验证通过。
- 同时观察到：华东/华中当时**正在下载中** —— 那是**用户本人在界面上点了更新**（见下），
  说明这条路径不是"只渲染不work"。
- `verify_packs.mjs --remote` 随后三段（`data/packs` / 用户目录 / 远端）**全绿、退出码 0**。

### 9.3 一次"看似自动下载"的排查（记录，避免下次误判）

现象：用户目录里 7 个包在 **02:39:29–41（12 秒内）**被写成了新版，华南失败并留下 0 字节 `.part`。

排除法（都有实测证据，不是推断）：
- ❌ **不是应用启动自动下载**：静默启动 30 秒（不碰任何界面）⇒ 包目录零变化、Rust 侧无输出。
- ❌ **不是本会话的 CDP 点击**：给 `window.__TAURI_INTERNALS__.invoke` 装了探针后逐个点
  「图层控制 / 电力设施 / 电压分级 / 设置」⇒ 每次点击 `pack_download` 调用数 **0**。
- ✅ **是用户在界面上自行触发的更新**（时间点正好在我把"本机 7 个包仍是上一版"报告出去之后；
  写入顺序与清单顺序一致、GEM 也在队列里）。
- 处置：华南用 `node scripts/install_packs.mjs --user-dir --only huanan` 补齐，并删掉那个 0 字节 `.part`。

⇒ 结论：**该行为符合预期**（G8 的"更新"按钮按下去就是逐包原子替换），不需要修代码。
