...
## 7. 未决项（**仅两项，且都必须在动手前探测**）

| # | 未决项 | 为什么未决 | 探测方式 |
|---|---|---|---|
| U1 | 中国 OSM 里 `power=converter` 的实际数量与分布 | 决定 G3 的规模（是否值得单独成层，还是并入变电站） | 用项目自己的抓取脚本对大范围 bbox 跑一次 `[power=converter]` 计数 |
| U2 | 中国 OSM 里 `[power=line][frequency]` 的覆盖率 | 决定 G5 走"frequency 判定"还是退化为"换流站拓扑判定" | 同上，跑一次 `[power=line][frequency]` 计数 |
